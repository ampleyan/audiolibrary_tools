...
export default function DiscoveryView({ onNavigate }: { onNavigate: (tab: string) => void }) {
  ...
  return (
    <div className="view discovery-view" style={{ padding: 24, color: "#f9fafb" }}>
      ...
      {loading ? <p style={{ color: "#4b5563", fontSize: 14 }}>Loading…</p> : tracks.length === 0 ? <p style={{ color: "#4b5563", fontSize: 14 }}>Add tracks to the Library before exploring related music.</p> : <SimilarPanel tracks={tracks} onOpenLibrary={() => onNavigate("library")} />}
    </div>
  );
}
