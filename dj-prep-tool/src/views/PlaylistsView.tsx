...
export default function PlaylistsView({ onNavigate }: { onNavigate?: (tab: string) => void }) {
  ...
  const sortedTracks = [...tracks].sort((left, right) => (Date.parse(right.updated_at) || 0) - (Date.parse(left.updated_at) || 0) || right.id - left.id);
